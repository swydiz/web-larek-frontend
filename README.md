...
Слой отображения отвечает за отрисовку пользовательского интерфейса и обработку взаимодействий пользователя.
    ...
    eventEmitter: EventEmitter - Экземпляр класса EventEmitter для обмена событиями.
    Методы:
    render(cartItemElements: HTMLElement[]): void - Отрисовывает список товаров в корзине, принимая массив HTML-элементов карточек.
    ...
    onAction?: (actionType: string, event: Event) => void - Функция обратного вызова, которая будет вызвана при наступлении определенного действия внутри карточки. Конструктор класса должен принимать параметры `template` и `onAction`.
    Методы:
    render(data: Product | CartItem): HTMLElement - Создает и возвращает HTML-элемент карточки товара, заполненный данными. В этом методе происходит поиск элементов внутри шаблона (например, `.card__title`, `.card__image`, `.card__price`) и установка их значений на основе данных товара.
    ...
    Методы:
    render(): void - Отображает сообщение об успешном оформлении заказа.
    setTotalPrice(totalPrice: number): void - Отображает общую стоимость заказа.
    hide(): void - Скрывает сообщение об успехе. Обработчик клика по кнопке "За новыми покупками".
